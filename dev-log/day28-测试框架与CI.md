...
> **今日目标**：从 Day 27 的代码出发，逐步动手完成 8 项改进：给 Connection 加回压保护、给 TcpServer 加连接上限、让 Socket 返回 bool、修复 pipeline 半丢请求、建立 app_example 示例工程，并接入 GitHub Actions CI 持续验证。
> **基于**：Day 27（调试与性能优化）。
...
在 Day 27 的代码基础上，今天你将逐步完成 8 项改进，每一步都可以独立编译验证。跟着下面的顺序动手，读完 §2—§9 你将建立起一套可在无网络环境中运行的单元测试体系。

**构建清单（按顺序）：**

1. **§2** — 给 `Connection` 加回压保护：写 `BackpressureConfig`、`BackpressureDecision`、`evaluateBackpressure()`
2. **§3** — 给 `TcpServer` 加连接上限：写 `Options` 结构体 + `shouldRejectNewConnection()` 纯函数
3. **§4** — 让 `Socket` 操作返回 `bool`：把 `bind()`/`listen()` 从 `void` 改为可检查的返回值
4. **§5** — 给 `EpollPoller` 加自愈重试：处理 fd 复用引起的 `EEXIST`/`ENOENT`
5. **§6** — 修复 HTTP pipeline 半丢请求：给 `HttpContext::parse()` 加 `*consumedBytes` 出参
6. **§7** — 整理 `HttpServer Options`：统一配置项并向下透传到 `Connection`
7. **§8** — 建立 `app_example` 独立示例工程：演示用 `find_package` 引用本库
8. **§9** — 接入 GitHub Actions CI：Linux + macOS 矩阵构建 + `ctest` 自动触发

**说明**：代码块前的「来自 `HISTORY/day28/...`」标注意为「将以下代码写入该文件的对应位置」，跟着每步动手输入即可。

---

## 1. 今天要解决的几个问题

### 1.1 Day 27 留下的五个具体痛点
...
## 6. 第 5 步 — 修复 HttpContext 的 pipeline 半丢问题
...
## 7. 第 6 步 — 整理 HttpServer Options

### 7.1 打开 HttpServer.h：定义 Options
